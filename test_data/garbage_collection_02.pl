...
reward(dropS(Robot, Item), Situation, Reward) :- Reward = 1,
													itemLocation(Item, 1, 0, Situation),
													robotItemLoad(Robot, Item, Situation)
												 ;
												 Reward = 0 .
...
getSituationSize(do(Action, Situation), Size) :- getSituationSize(Situation, OldSize),
													Size is OldSize + 1.
getSituationSize(s0, Size) :- Size is 0. 					

getProgramLength(Statement : Statements, ProgramLength) :- getProgramLength(Statements, RemainingProgramLength),
																ProgramLength is RemainingProgramLength + 1.
getProgramLength(nil, ProgramLength) :- ProgramLength is 0.

constructNiledPrograms(Prog, Level) :- SubLevel is Level - 1, SubLevel >= 0,
								constructNiledPrograms(SubProg, SubLevel),
								(Prog = (moveForward(robot0) : SubProg);
									Prog = (turnLeft(robot0) : SubProg);
									Prog = (turnRight(robot0) : SubProg);
									Prog = (stop(robot0) : SubProg);
									Prog = (pickUp(robot0, item0) : SubProg);
									Prog = (drop(robot0, item0) : SubProg)
									).
constructNiledPrograms(Prog, 0) :- Prog = nil.

constructPrograms(Prog, Level) :- SubLevel is Level - 1, SubLevel >= 0,
								constructPrograms(SubProg, SubLevel),
								constructPrograms(Instruction, 0),
								Prog = (Instruction : SubProg).
constructPrograms(Prog, 0) :- constructInstruction(Instruction), Prog = Instruction .				
constructInstruction(Instruction) :- Instruction = moveForward(robot0);
								Instruction = turnLeft(robot0);
								Instruction = turnRight(robot0);
								Instruction = stop(robot0);
								Instruction = pickUp(robot0, item0);
								Instruction = drop(robot0, item0).


testProgs(Prog, Size, Probability, Situation) :- constructNiledPrograms(Prog, Size),
													stDo(Prog, Probability, s0, Situation).

eValuePrograms(MaxSize) :- between(1, MaxSize, Size),
										constructPrograms(Prog, Size),
										eValue(Prog, Value),
										Value > 0,
										write('Program: '), write(Prog), nl,
										write('Expected value: '), write(Value), nl,
										fail.
										
%eValueBestProgram(Prog, MaxSize, MaxValue) :- ...
